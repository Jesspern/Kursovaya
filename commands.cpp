...
void add_node_key::execute()
{
	...
	key_contest->set_id_member(_value->get_id_member());
	key_contest->set_id_contest(_value->get_id_contest());
	int checker = 0;
	try {
		_tree->find(key_contest);
	}
	catch (std::invalid_argument&) {
		checker = 1;
	}
	if (checker) {
		_tree->insert(key_contest, _value);
	}
	else {
...
void read_node_key::execute()
{
	contest_info* value;
	contest_info* key_contest = new contest_info;
	key_contest->set_id_member(_key.first);
	key_contest->set_id_contest(_key.second);
	try {
		value = _tree->find(key_contest);
	}
	catch (std::invalid_argument&) {
...
void remove_node_key::execute()
{
	...
	key_contest->set_id_contest(_key.second);
	try {
		value = _tree->remove(key_contest);
	}
	catch (std::invalid_argument&) {
		throw std::invalid_argument("ID is not correct");
	}
...
void read_node_keys::execute()
{
	...
	contest_info* value;
	contest_info* key_contest1 = new contest_info;
	key_contest1->set_id_member(_key1.first);
	key_contest1->set_id_contest(_key1.second);
	contest_info* key_contest2 = new contest_info;
	key_contest2->set_id_member(_key2.first);
	key_contest2->set_id_contest(_key2.second);
	try {
		value = _tree->find(key_contest1);
	}
	catch (std::invalid_argument&) {
		throw std::invalid_argument("ID is not correct");
	}
	if (dynamic_cast<Btree<contest_info*, contest_info*, comparer_id_member_id_contest>*>(_tree)) {
		Btree<contest_info*, contest_info*, comparer_id_member_id_contest>* _btree = dynamic_cast<Btree<contest_info*, contest_info*, comparer_id_member_id_contest>*>(_tree);
		command* temp;
		contest_info copy_value;
		auto it = _btree->infix_iterator_begin();
		for (it; it != _btree->infix_iterator_end(); ++it) {
			if (_comp.comparate(std::get<0>(*it), key_contest2) == 1) {
				break;
			}
			if (_comp.comparate(std::get<0>(*it), key_contest2) == 2 or _comp.comparate(std::get<0>(*it), key_contest2) == 0) {
		...
		if (_comp.comparate(std::get<0>(*it), key_contest2) == 0) {
			//std::cout << (std::get<1>(*it)) << std::endl;
			temp = (std::get<1>(*it))->first_command;
			copy_value = *(std::get<1>(*it));
			if (temp == nullptr or temp->_time > _time) {
